...
const Dashboard = () => {
  ...
  const highPriorityCount = contacts.filter(c => c.priorityLevel === "high").length;
  const highPriorityPercent = contacts.length > 0 
    ? Math.round((highPriorityCount / contacts.length) * 100) 
    : 0;
  
  const activeDealsCount = accounts.reduce((sum, account) => sum + account.activeDeals, 0);
  const averagePenetration = accounts.length > 0
    ? Math.round(accounts.reduce((sum, account) => sum + account.penetrationScore, 0) / accounts.length)
    : 0;
    
  const recentSignalsCount = contacts.flatMap(c => 
